...
@interface CCWCoinTypeSheetView()<UITableViewDelegate,UITableViewDataSource>
// 背后控件
...
@implementation CCWCoinTypeSheetView

- (NSMutableArray *)dataSource
{
    if (!_dataSource) {
        _dataSource = @[@"CNY",@"USD",CCWLocalizable(@"取消")].mutableCopy;
    }
    return _dataSource;
}
...
{
    CCWCoinTypeSheetViewCell *cell = [CCWCoinTypeSheetViewCell cellWithTableView:tableView cellStyle:UITableViewCellStyleSubtitle WithIdentifier:@"myAlertSheetCell"];
    cell.titleLabel.text = self.dataSource[indexPath.row];
    cell.titleLabel.textAlignment = NSTextAlignmentLeft;
    if (indexPath.row == 0) {
        cell.coinTypeImageView.image = [UIImage imageNamed:CCWCNYORUSD?@"languageSelect":@"languageNomal"];
    }else if (indexPath.row == 1){
        cell.coinTypeImageView.image = [UIImage imageNamed:CCWCNYORUSD?@"languageNomal":@"languageSelect"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        [CCWSaveTool setBool:NO forKey:CCWCurrencyType];
    }else{
      [self CCW_Close];
    }
    if ([self.delegate respondsToSelector:@selector(CCW_CoinTypeSheetView:didSelectRowAtIndexPath:)]) {
        [self.delegate CCW_CoinTypeSheetView:self didSelectRowAtIndexPath:indexPath];
    }
