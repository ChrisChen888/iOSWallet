...
@interface CocosSwitchAccountView ()<UITableViewDelegate,UITableViewDataSource>
// 背后控件
...
@implementation CocosSwitchAccountView
...
-(UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel.text = [NSString stringWithFormat:@"COCOS%@",CCWLocalizable(CCWCoinTypeString)];
...
-(UIButton *)closeBtn{
    if (!_closeBtn) {
        ...
        [_closeBtn addTarget:self action:@selector(Cocos_CloseCompletion:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _closeBtn;
}
...
{
    CocosSwitchAccountCell *cell = [CocosSwitchAccountCell cellWithTableView:tableView WithIdentifier:@"invokerSwitchAccountCell"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(Cocos_SwitchAccountView:didSelectDBAccountModel:)]) {
        [self.delegate Cocos_SwitchAccountView:self didSelectDBAccountModel:self.dataSource[indexPath.row]];
    }
...
{
    CCWWeakSelf
//    [self Cocos_CloseCompletion:^(BOOL finished) {
//        if ([weakSelf.delegate respondsToSelector:@selector(Cocos_SwitchViewAddAccountClick:)]) {
//            [weakSelf.delegate Cocos_SwitchViewAddAccountClick:self];
//        }
//    }];
}
@end
